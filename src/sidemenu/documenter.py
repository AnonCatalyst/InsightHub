from PyQt6.QtWidgets import (
    QApplication, QWidget, QVBoxLayout, QPushButton, QTextEdit, QComboBox, QLineEdit,
    QMessageBox, QTabWidget, QHBoxLayout, QFileDialog, QStatusBar, QMenu, QInputDialog
)
...
class LoggingWindow:
    """Placeholder for the LoggingWindow class."""
...
class Documenter(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # Connect context menu to tab bar for showing custom context menu
        self.tab_widget.tabBar().setContextMenuPolicy(Qt.ContextMenuPolicy.CustomContextMenu)
        self.tab_widget.tabBar().customContextMenuRequested.connect(self.show_tab_context_menu)

        # Use QTimer to ensure window size is set after the event loop starts
        QTimer.singleShot(0, self.set_initial_window_size)

    def set_initial_window_size(self):
        """Set initial window size after the main event loop starts."""
        self.resize(900, 550)  # Width: 1000px, Height: 600px
    ...
    def rename_tab(self, tab_index):
        tab_widget = self.tab_widget.widget(tab_index)
        file_name_input = tab_widget.findChild(QLineEdit)
        new_name, ok = QInputDialog.getText(self, "Rename Document", "Enter new name:", text=file_name_input.text())

        if ok and new_name:
            file_name_input.setText(new_name)
...
if __name__ == "__main__":
    ...
    window = Documenter()    # Instantiate the Documenter window
